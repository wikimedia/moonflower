...
<div class="gacha-card-container" class:is-flipped={isFlipped}>
	{#if !isFlipped}
		...
		<div class="hover-3d" onclick={flip}>
			<div
				class="card gacha-card-shell card-back bg-neutral text-neutral-content flex w-full items-center justify-center border-2 border-base-content/20 cursor-pointer"
			>
	...
	{:else}
		<!-- Revealed card. Can be linkable or static depending on experiment behavior. -->
		{#if linkable}
			<!-- svelte-ignore a11y_click_events_have_key_events -->
			<!-- svelte-ignore a11y_no_static_element_interactions -->
			<div
