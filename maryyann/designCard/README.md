...
Mary Yann–scoped **non-UI** helpers and design notes for the Moonflower experiment at `/design-card`.

- User-facing strings live in `src/lib/i18n/en.ts` under `en.experiments.designCard`.
- Svelte UI lives in `src/routes/design-card/`.
- This folder holds TypeScript helpers (when added) and this plan document.
...
- [x] Add `maryyann/designCard/` TypeScript (aspect/bg constants + first-sentence helper from `WikiArticle.extract`)
- [x] Create `src/routes/design-card/+layout.svelte` and `+page.svelte` (pull with `requireImages`, card layout **70/30**, cover image, 20px radius)
- [x] Add `en.experiments.designCard` strings and `experiments.ts` hub entry (slug `design-card`)
